import { RiAddFill, RiAsterisk, RiFilter3Fill, RiMenuLine } from "@remixicon/react";
...
/**
 * Figma source: Board UI → medical profile dashboard → Frame 60 (node
 * 3950:5627, "Medical Profile" breadcrumb icon is Remix `asterisk`) + Frame
 * 50 (node 3950:5645) for the title/actions row — same breadcrumb + title +
 * action-row recipe as `DashboardHeader` and `CalendarHeader`.
 */
export function MedicalHeader({ onMenuClick }: { onMenuClick?: () => void } = {}) {
  return (
    <header className="flex w-full flex-col gap-2">
      ...
      <Breadcrumb>
        ...
        <BreadcrumbItem current icon={RiAsterisk}>
          Medical Profile
        </BreadcrumbItem>
      </Breadcrumb>
