...
export type T3WorkEntryTone = "thinking" | "tool" | "info" | "error";

export type T3WorkEntryLifecycleStatus =
  | "inProgress"
  | "completed"
  | "failed"
  | "declined"
...
export type T3WorkEntryItemType =
  | "command_execution"
  | "file_change"
  | "web_search"
  | "image_view"
...
export type T3WorkEntryRequestKind = "command" | "file-read" | "file-change";

/** Upstream `WorkLogEntry`, trimmed to the presentation fields this slice renders. */
export interface T3WorkEntry {
  id: string;
  label: string;
  tone: T3WorkEntryTone;
  detail?: string;
  command?: string;
  rawCommand?: string;
  ...
  toolTitle?: string;
  toolData?: unknown;
  itemType?: T3WorkEntryItemType;
  requestKind?: T3WorkEntryRequestKind;
  toolLifecycleStatus?: T3WorkEntryLifecycleStatus;
...
export function workEntryIsToolLike(entry: T3WorkEntry): boolean {
...
export function workEntryIndicatesToolFailure(entry: T3WorkEntry): boolean {
  if (entry.tone === "error") {
    return true;
  }
...
export function workEntryIndicatesToolSuccess(entry: T3WorkEntry): boolean {
...
export function workEntryIndicatesToolNeutralStatus(entry: T3WorkEntry): boolean {
  if (!workEntryIsToolLike(entry)) return false;
  if (workEntryIndicatesToolFailure(entry)) return false;
...
export function workEntryPreview(
  entry: Pick<T3WorkEntry, "detail" | "command" | "changedFiles">,
  workspaceRoot: string | undefined,
): string | null {
...
export function workEntryRawCommand(
  entry: Pick<T3WorkEntry, "command" | "rawCommand">,
): string | null {
...
export type T3WorkEntryIconName =
  | "bot"
  | "check"
  | "circle-alert"
  | "eye"
...
export function workToneIconName(tone: T3WorkEntryTone): T3WorkEntryIconName {
...
export function workEntryIconName(entry: T3WorkEntry): T3WorkEntryIconName {
...
export interface T3WorkEntryOverflow {
