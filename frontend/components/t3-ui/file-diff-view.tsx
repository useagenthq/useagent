...
// T3ChangedFilesCard as the file index, T3DiffStatLabel for honest line stats).
//
...
import { CHANGED_FILES_AUTO_EXPAND_FILE_LIMIT, type T3ChangedFile } from "./changed-files";
import { T3ChangedFilesCard } from "./changed-files-tree";
import { hasNonZeroStat, T3DiffStatLabel } from "./diff-stat-label";
...
const T3FileDiffSection = memo(function T3FileDiffSection(props: {
  file: T3ChangedFile;
      ...
      >
        ...
        {stat && hasNonZeroStat(stat) && (
          <span className="shrink-0 font-mono text-[10px] tabular-nums">
            <T3DiffStatLabel additions={stat.additions} deletions={stat.deletions} layout="inline" />
          </span>
        )}
      </button>
 ...
 */
export function T3FileDiffView({
  files,
  patches,
}: {
  ...
  return (
    <div data-t3-ui="file-diff-view" className="space-y-3 p-3 pt-0">
      <T3ChangedFilesCard files={files} showCompactPreview={false} onOpenFile={openFile} />
      {files.map((file) => (
        <T3FileDiffSection
          key={file.path}
          file={file}
