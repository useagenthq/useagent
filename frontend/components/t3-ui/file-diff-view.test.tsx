...
describe("T3FileDiffView", () => {
  ...
  test("renders the changed-files index over per-file unified diff sections", () => {
    const html = render(<T3FileDiffView files={FILES} patches={PATCHES} />);
    expect(html).toContain('data-t3-ui="file-diff-view"');
  ...
  test("renders nothing without changed files", () => {
    expect(render(<T3FileDiffView files={[]} patches={new Map()} />)).toBe("");
  });
});
