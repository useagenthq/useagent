...
const ENTRY_ICON: Record<T3WorkEntryIconName, RemixiconComponentType> = {
  bot: RiRobot2Line,
  check: RiCheckLine,
...
function workToneClass(tone: T3WorkEntry["tone"]): string {
  return tone === "info" ? "text-text-soft-400" : "text-text-strong-950";
}

function StatusIndicator({ entry, turnSettled }: { entry: T3WorkEntry; turnSettled: boolean }) {
 ...
 */
export const T3WorkEntryRow = memo(function T3WorkEntryRow({
  entry,
  workspaceRoot,
  turnSettled = true,
}: {
