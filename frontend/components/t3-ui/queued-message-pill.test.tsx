...
import { queuedPillLabel, T3QueuedMessagePill } from "./queued-message-pill";
...
test("head queued pill is right-aligned and carries the Send now steering", () => {
  const html = renderToStaticMarkup(<T3QueuedMessagePill position={1} onSendNow={() => {}} />);
...
test("non-head pill shows its position without the steering affordance", () => {
  const html = renderToStaticMarkup(<T3QueuedMessagePill position={2} />);
