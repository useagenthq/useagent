...
test("short plan renders fully with no expand toggle and no approve action", () => {
  const html = renderToStaticMarkup(<T3ProposedPlanCard planMarkdown={SHORT_PLAN} />);
  expect(html).toContain('data-t3-ui="proposed-plan-card"');
...
test("long plan collapses to the preview behind the toggle", () => {
  const html = renderToStaticMarkup(<T3ProposedPlanCard planMarkdown={LONG_PLAN} />);
...
test("defaultExpanded shows the full plan and the collapse toggle", () => {
  const html = renderToStaticMarkup(
    <T3ProposedPlanCard planMarkdown={LONG_PLAN} defaultExpanded />,
  );
...
test("onImplement surfaces the approve action", () => {
  const html = renderToStaticMarkup(
    <T3ProposedPlanCard planMarkdown={SHORT_PLAN} onImplement={() => {}} />,
  );
  expect(html).toContain("Implement plan");
});

test("renders nothing for an empty plan", () => {
  expect(renderToStaticMarkup(<T3ProposedPlanCard planMarkdown={"  \n "} />)).toBe("");
});
