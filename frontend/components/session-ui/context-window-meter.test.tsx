...
test("meter ring reports percent used and turns error-red past 90%", () => {
  const steady = render(
    <ContextWindowMeter usage={{ usedTokens: 61_400, maxTokens: 200_000 }} />,
  );
...
test("meter without a known limit reports raw tokens instead of a percent", () => {
  const html = render(<ContextWindowMeter usage={{ usedTokens: 61_400, maxTokens: null }} />);
...
test("details panel without a limit shows tokens only, no progressbar", () => {
  const html = renderToStaticMarkup(
    <ContextWindowDetails usage={{ usedTokens: 61_400, maxTokens: null }} />,
  );
