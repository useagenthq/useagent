...
// no checkpoint lane, so ChangedFile carries the same shape with the stats
...
function readStat(file: ChangedFile): TurnDiffStat | null {
...
export function summarizeTurnDiffStats(files: ReadonlyArray<ChangedFile>): TurnDiffStat {
  return files.reduce(
    (acc, file) => {
...
export function buildTurnDiffTree(files: ReadonlyArray<ChangedFile>): TurnDiffTreeNode[] {
