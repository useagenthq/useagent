...
export function SessionView({ initialThread, initialOutline = null, initialRelationshipHint = "legacy_or_off", assistantIdentity, readOnlyMessage, onNewestTurnChange }: {
  ...
  // Gateway approvals (#77): the turns' approval SIGNATURES (not a poll) drive
  // the thread-scoped fetch of pending records from GET /api/gateway/approvals;
  // an approval event arriving on the thread SSE re-projects the turn, changes
  // the signature, and triggers exactly one refetch (see use-gateway-approvals).
