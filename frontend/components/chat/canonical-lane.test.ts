...
function ev(
  over: Partial<StoredCanonicalEvent> & { eventId: string; deliverySeq: number },
...
function childEvent(over: {
  ...
  const kind = over.kind ?? "child.started";
  return {
    schemaVersion: 1,
    eventId: over.eventId,
    seq: over.deliverySeq,
    runId: over.runId ?? RUN,
    threadId: over.threadId ?? RUN,
    ts: 1_000 + over.deliverySeq,
    deliverySeq: over.deliverySeq,
    revision: over.revision ?? 0,
    identity: { provider: "codex", nativeSessionId: "parent" },
    kind,
    childId: over.childId,
    ...(kind === "child.started" ? { title: over.childId } : {}),
    ...(kind === "child.updated" ? { status: "running" } : {}),
    ...(kind === "child.completed" ? { status: "ok" as const } : {}),
  } as StoredCanonicalEvent;
}
...
describe("thread store canonical lane (slice 4)", () => {
  test("accumulates per-run canonical events, ordered by deliverySeq", () => {
    ...
    if (!view) throw new Error("expected run view");
  ...
  test("the accumulated lane feeds buildTimelineFromCanonical", () => {
    ...
    store.applyCanonical(
      ev({
        ...
        messageId: "m1",
        identity: { nativeSessionId: "ses_root", nativeSeq: 0, nativeMessageId: "m1" },
      } as never),
    );
    store.applyCanonical(
      ev({
      ...
      [
        "s0",
        {
          ...
          code_json: JSON.stringify({
            tool: "bash",
            type: "tool",
            native: { messageID: "m1", partID: "p", callID: "c" },
          }),
          created_at: "",
  ...
  test("owns one stable incremental execution summary outside render", () => {
    const store = createThreadStore({ rootThreadId: RUN, executionSummaryEnabled: true });
    store.applySnapshot([run]);
    const empty = store.getExecutionSummarySnapshot();
    expect(empty?.children).toEqual([]);
    expect(store.getExecutionSummarySnapshot()).toBe(empty);

    const started = childEvent({ eventId: "child", deliverySeq: 1, childId: "child-a" });
    store.applyCanonical(started);
    const accepted = store.getExecutionSummarySnapshot();
    expect(accepted?.children.map((child) => child.id)).toEqual(["child-a"]);
    expect(store.getExecutionSummarySnapshot()).toBe(accepted);
    expect(store.getSnapshot().executionSummary).toBe(accepted);

    store.applyCanonical(started);
    expect(store.getExecutionSummarySnapshot()).toBe(accepted);
    store.applyNative(RUN, {
      schemaVersion: 1,
      eventId: "native-only",
      seq: 1,
      provider: "codex",
  ...
  test("feeds only accepted revisions and removes superseded lifecycle identity", () => {
    const store = createThreadStore({ rootThreadId: RUN, executionSummaryEnabled: true });
    store.applySnapshot([run]);
    store.applyCanonical(
      childEvent({
    ...
    const revised = store.getExecutionSummarySnapshot();
    expect(revised?.children.map((child) => child.id)).toEqual(["child-b"]);
    ...
    expect(store.getExecutionSummarySnapshot()).toBe(revised);
    expect(store.getSnapshot().byId.get(RUN)?.canonical).toHaveLength(1);
  });

  test("keeps root stores and per-run execution summaries isolated", () => {
    const rootA = createThreadStore({ rootThreadId: "root-a", executionSummaryEnabled: true });
    const rootB = createThreadStore({ rootThreadId: "root-b", executionSummaryEnabled: true });
    const runA = { ...run, id: "run-a", thread_id: "root-a" };
    const runB = { ...run, id: "run-b", thread_id: "root-b" };
    rootA.applySnapshot([runA]);
    rootB.applySnapshot([runB]);
    rootA.applyCanonical(
      childEvent({
        eventId: "a",
        deliverySeq: 1,
        childId: "child-a",
    ...
    expect(rootA.getSnapshot().executionSummary?.children.map((child) => child.id)).toEqual([
      "child-a",
    ]);
    expect(rootB.getSnapshot().executionSummary?.children.map((child) => child.id)).toEqual([
      "child-b",
    ]);
    expect(rootA.getSnapshot().byId.get("run-a")?.executionSummary?.children).toHaveLength(1);
    expect(rootB.getSnapshot().byId.get("run-b")?.executionSummary?.children).toHaveLength(1);
  });

  test("off mode constructs no execution-summary read state", () => {
    const store = createThreadStore({ rootThreadId: RUN, executionSummaryEnabled: false });
    store.applySnapshot([run]);
    store.applyCanonical(childEvent({ eventId: "child", deliverySeq: 1, childId: "child-a" }));
    expect(store.getExecutionSummarySnapshot()).toBeNull();
    expect(store.getSnapshot().executionSummary).toBeNull();
    expect(store.getSnapshot().byId.get(RUN)?.executionSummary).toBeNull();
  });

  test("projection failure cannot corrupt the accepted legacy lane", () => {
    const store = createThreadStore({ rootThreadId: RUN, executionSummaryEnabled: true });
    store.applySnapshot([run]);
    store.applyCanonical(
      childEvent({
        eventId: "cross-thread",
        deliverySeq: 1,
        childId: "child-a",
        threadId: "other-root",
      }),
...
describe("canonical SSE frame validation (H4 - issue #6)", () => {
  ...
  const valid = (): Record<string, unknown> => ({
    ...
    deliverySeq: 5,
    revision: 0,
    identity: { nativeSessionId: "ses_root" },
    text: "hi",
  });
  ...
  test("validateCanonicalComplete: needs a runId, rejects a cross-thread payload", () => {
    expect(validateCanonicalComplete({ runId: "run_v", threadId: THREAD }, THREAD)).toEqual({
      runId: "run_v",
    });
    expect(validateCanonicalComplete({ runId: "" }, THREAD)).toBeNull();
    expect(validateCanonicalComplete(null, THREAD)).toBeNull();
    expect(
      validateCanonicalComplete({ runId: "run_v", threadId: "thread_other" }, THREAD),
    ).toBeNull();
  });
});
...
describe("canonical render gate (H2+H4 flag-on / partial-canonical)", () => {
  ...
  test("flag OFF never uses canonical, even when complete with events", () => {
    expect(shouldUseCanonicalTimeline(false, { canonical: events, canonicalComplete: true })).toBe(
      false,
    );
  });
  test("flag ON but NOT complete falls back to legacy (partial-canonical never renders)", () => {
    expect(shouldUseCanonicalTimeline(true, { canonical: events, canonicalComplete: false })).toBe(
      false,
    );
  ...
  test("flag ON + complete but EMPTY lane falls back to legacy", () => {
    expect(shouldUseCanonicalTimeline(true, { canonical: [], canonicalComplete: true })).toBe(
      false,
    );
  });
  test("flag ON + complete + events uses canonical", () => {
    expect(shouldUseCanonicalTimeline(true, { canonical: events, canonicalComplete: true })).toBe(
      true,
    );
  });
});
