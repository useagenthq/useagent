...
// durable requests are read once on mount, then re-read only when a turn's
...
// signature -> one revalidate). The fetch is THREAD-scoped and carries every
// status: a request whose run has settled (the agent ended its turn to wait, as
// approval_request tells it to) is still the thread's to decide, and a card that
// was approved or denied is thread history that survives reload. `refresh` is
// the manual nudge after this client resolves (or loses a 409 race on) an approval.
