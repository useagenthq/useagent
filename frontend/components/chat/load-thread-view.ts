...
import {
  type InitialThreadRelationshipHint,
  loadThreadRelationshipHint,
} from "@/lib/thread-relationship-hint";
...
async function loadFullThread(id: string, fetcher: typeof backendFetch): Promise<ApiRun[]> {
  try {
    const res = await fetcher(`/api/runs/${id}?thread=1`);
    if (!res.ok) return [];
...
async function loadWindowedTail(
  ...
  fetcher: typeof backendFetch,
): Promise<ApiRun[]> {
  try {
    const requested = initialTurnIds(outline);
    const chunks = chunkTurnIds(requested);
    const responses = await Promise.all(
      chunks.map((chunk) => fetcher(`/api/runs/${id}/turns?ids=${chunk.join(",")}`)),
    );
    const runs: ApiRun[] = [];
...
export async function loadThreadView(
  id: string,
  relationshipLookupApplicable: boolean | Promise<boolean> = true,
  fetcher: typeof backendFetch = backendFetch,
): Promise<ThreadView | null> {
  let outline: ApiThreadOutlineTurn[] = [];
  try {
    const res = await fetcher(`/api/runs/${id}/thread-outline`);
  ...
  if (outline.length > WINDOWED_THREAD_LIMIT) {
    const tail = await loadWindowedTail(id, outline, fetcher);
    if (tail.length > 0) {
      const relationshipHint = await loadThreadRelationshipHint(
        tail[0]?.thread_id ?? id,
        fetcher,
        await relationshipLookupApplicable,
      );
  ...
  const thread = await loadFullThread(id, fetcher);
  if (thread.length === 0) return null;
  const relationshipHint = await loadThreadRelationshipHint(
    thread[0]?.thread_id ?? id,
    fetcher,
    await relationshipLookupApplicable,
  );
