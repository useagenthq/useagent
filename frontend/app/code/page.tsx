...
export default function CodePage() {
  return (
    <AppShell sidebar={<FileTreeRail />}>
      <div className="mx-auto w-full max-w-4xl p-6 lg:p-8">
        {/* File breadcrumb + live editing status */}
        <div className="flex flex-wrap items-center justify-between gap-3">
          <nav aria-label="File path" className="flex items-center gap-1 text-label-sm">
            {crumbs.map((crumb) => (
              <span key={crumb} className="flex items-center gap-1">
                <span className="text-text-sub-600">{crumb}</span>
                <RiArrowRightSLine className="size-4 text-text-soft-400" aria-hidden />
              </span>
            ))}
            <span className="inline-flex items-center gap-1.5 text-text-strong-950">
              <RiReactjsLine className="size-4 text-primary-base" aria-hidden />
              run-trace.tsx
            </span>
          </nav>
          ...
          <div className="flex shrink-0 items-center gap-2">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-success-lighter py-1 pl-1.5 pr-2.5 text-label-xs text-success-base">
              <span className="size-2 rounded-full bg-success-base" aria-hidden />
              Agent editing
            </span>
            <span className="text-label-xs tabular-nums text-text-soft-400">6m 12s</span>
          </div>
        </div>
