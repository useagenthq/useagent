...
function ShellCard({
  ...
  return (
    <section
      ...
      className={cn(
        "flex flex-col gap-4 rounded-3xl border border-stroke-soft-200 bg-bg-white-0 p-5 shadow-regular-xs",
        wide ? "lg:col-span-2" : "",
      )}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="font-mono text-label-xs text-text-soft-400">{slug}</p>
          <h3 className="mt-1 text-title-h6 text-text-strong-950">{title}</h3>
          <p className="mt-1 text-paragraph-sm text-text-sub-600">{note}</p>
        </div>
      ...
      <div>{children}</div>
      <div className="flex flex-wrap items-center justify-between gap-2 text-paragraph-xs text-text-soft-400">
        ...
        {sourceUrl ? (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-1 rounded-full border border-stroke-soft-200 bg-bg-weak-50 px-2 py-1 text-label-xs text-text-sub-600 transition-colors hover:bg-bg-soft-200"
          >
            Source
            <RiExternalLinkLine className="size-3" aria-hidden />
          </a>
        ) : (
          <span className="rounded-full border border-stroke-soft-200 bg-bg-weak-50 px-2 py-1 text-label-xs">
            Local primitive
          </span>
        )}
      </div>
...
export function BeuiAgentInventory() {
  ...
  return (
    <section className="border-t border-stroke-soft-200 py-10">
        ...
        <div className="bg-halftone flex flex-col gap-5 p-6 sm:p-8">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="max-w-2xl">
              <div className="flex flex-wrap items-center gap-2">
                <span className="flex size-10 items-center justify-center rounded-2xl bg-primary-base text-static-white">
                  <RiSparkling2Line className="size-5" aria-hidden />
                </span>
                <div>
                  <p className="font-mono text-label-xs text-text-disabled-300">
                    {BEUI_AGENT_SOURCE_URL}
                  </p>
                  <h2 className="mt-1 text-title-h4 text-text-white-0">beUI agent inventory</h2>
                </div>
              </div>
              <p className="mt-3 text-paragraph-md text-text-disabled-300">
            ...
            <div className="flex flex-col items-end gap-2 text-right">
              <span className="rounded-full bg-bg-white-0/10 px-3 py-1 text-label-xs text-text-white-0">
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-1 rounded-full border border-stroke-strong-400 px-3 py-1.5 text-label-xs text-text-white-0 transition-colors hover:bg-bg-surface-800"
              >
                Registry
          ...
          <div className="flex flex-wrap gap-2">
            {BEUI_REJECTED_SURFACES.map((item) => (
              <span
                key={item.slug}
                className="inline-flex items-center gap-1 rounded-full border border-stroke-strong-400 px-2.5 py-1 text-label-xs text-text-disabled-300"
              >
                {item.label}
                <span className="text-text-soft-400">rejected</span>
              </span>
            ))}
          </div>
        </div>
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-2">
        <ShellCard
          ...
          wide
        >
          <div className="rounded-3xl border border-stroke-soft-200 bg-bg-white-0 p-3 shadow-regular-xs">
            <Composer
              variant="compact"
