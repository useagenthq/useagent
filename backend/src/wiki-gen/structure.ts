...
export function parseWikiStructure(text: string, comprehensive: boolean): WikiStructure {
  let stripped = text.trim().replace(/^```(?:xml)?\s*/i, "").replace(/```\s*$/, "");

  const match = stripped.match(/<wiki_structure>[\s\S]*?<\/wiki_structure>/i);
  if (!match) {
  ...
  const xml = match[0].replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, "");
