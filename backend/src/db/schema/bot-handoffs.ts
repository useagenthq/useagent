...
export const botHandoffs = pgTable(
  ...
  (t) => [
    primaryKey({ name: "bot_handoffs_org_thread_pk", columns: [t.orgId, t.threadId] }),
    index("idx_bot_handoffs_bot").on(t.orgId, t.botId, t.createdAt),
    foreignKey({ name: "fk_bot_handoffs_bot", columns: [t.botId], foreignColumns: [bots.id] }).onDelete("cascade"),
    foreignKey({ name: "fk_bot_handoffs_thread", columns: [t.orgId, t.threadId], foreignColumns: [runs.orgId, runs.id] }).onDelete("cascade"),
  ],
);
