 ...
 *
 * Isolation (NEVER the shared stack): throwaway DB skynet_e2e_timings, own backend
 * on :3515, a cloudflared tunnel so the sandbox can reach the provider gateway.
 ...
 * thread sandbox (API-verified), drops the DB, kills the backend + tunnel.
 *
...
async function startBackend(publicUrl: string): Promise<Proc> {
  ...
  const proc = Bun.spawn(["bun", "src/index.ts"], {
    ...
    env: {
      ...
      // Provider gateway wired via the tunnel (OpenCode hard-requires it).
...
async function establishTunnel(): Promise<PublicTunnel> {
  ...
  for (const provider of providers) {
    try {
      const t = await startPublicTunnel({ localPort: PORT, logPath: tunnelLog, provider });
...
async function main(): Promise<void> {
  ...
  console.log("  establishing a public tunnel to the isolated backend…");
  ...
  let ok = false;
  try {
    proc = await startBackend(tunnel.publicUrl);
    ...
    // The tunnel -> backend gateway path is exercised by the live sandbox turn
    ...
    console.log(`  gateway path: sandbox -> ${tunnel.publicUrl}/api/provider -> backend (validated by the turn)`);

    // ── Turn 1: COLD create ────────────────────────────────────────────────────
    console.log("\n── Turn 1 (COLD: fresh sandbox, full config activation) ──");
    const t1 = await createRun({ prompt: "T1 Reply with a one-word greeting and nothing else.", engine: "opencode", model: MODEL });
    ...
    const t2 = await createRun({ prompt: "T2 Reply with a one-word farewell and nothing else.", engine: "opencode", model: MODEL, parent_run_id: t1 });
    ...
    if (proc) await killBackend(proc).catch(() => {});
