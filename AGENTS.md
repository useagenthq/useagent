...
substrate. It must not change resume, cancel, approvals, questions, child
events, or canonical projection semantics.

There is no compatibility transport. If a sandbox provider cannot host an
engine's native runtime, report that engine/provider pair as unsupported and
stop. Do not silently substitute another engine or a reduced lifecycle.
