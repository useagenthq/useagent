...
| Sandbox/runtime names such as `skynet-run`, `skynet-agent-v17`, `skynet-thread-*`, and `skynet://` references | Active snapshot, inventory-label, native-session, and event-reference contracts. | Roll out new producers and dual-read consumers before relabeling existing sandboxes. |
