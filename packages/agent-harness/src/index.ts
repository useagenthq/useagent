/**
 * @skynet/agent-harness - server-side agent harness contracts.
 *
 ...
 * Daytona, or the React UI. The browser-facing `@skynet/agent-client` may import
 * only the zero-dependency `./canonical` subpath from here, never a translator.
 *
 * Subpath entry points (see package.json exports):
 *   - `@skynet/agent-harness/canonical` - the pure canonical event vocabulary
 *   - `@skynet/agent-harness/control`   - the pure HarnessAdapter control types
 *   - `@skynet/agent-harness/opencode`  - the OpenCode -> canonical translator
 */
