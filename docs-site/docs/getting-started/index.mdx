...
description: Understand the frontend, backend, shared packages, and durable command flow before starting a local useAgent stack.
sidebar:
  label: Overview
  order: 1
---
...
<div style={{ background: "#ffffff", border: "1px solid #ebebeb", borderRadius: "12px", padding: "12px", margin: "16px 0 8px" }}>
  <img
    ...
    alt="The command-lane round trip: Web UI, Slack, or an automation becomes a durable command, a worker steers a native engine session in a sandbox, native frames are canonicalized into the Postgres event log, and a thread-scoped SSE stream renders the session timeline."
...
<Steps>
  <Step title="A command enters the lane">
    ...
    command. When the caller supplies an idempotency key, an exact retry returns
    the original acceptance instead of duplicating work.
  </Step>
  <Step title="A run executes in a sandbox">
    The backend claims a sandbox, steers the thread's native engine session, and
    streams its output into the provider and canonical event lanes.
  </Step>
...
:::tip
The runtime is **Bun** everywhere. The two apps and the `packages/*` modules each
