...
Skynet is a multi-package repository with one product UI, one backend control
...
<CardGroup cols={3}>
  <Card title="Frontend" icon="monitor">
    ...
    artifacts, secrets, learnings review, automations, and settings. Browser
...
:::tip
The runtime is **Bun** everywhere. The two apps carry their own lockfiles, so you
install inside each app rather than at the root.
:::
