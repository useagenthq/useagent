...
| Method | Path | Description | Notes |
| --- | --- | --- | --- |
| GET | `/api/commands` | The native slash-command catalog for "/" autocomplete before a sandbox exists. | Query `engine` to read the latest catalog that engine advertised in the current org's canonical stream; defaults to `opencode`. Returns `{engine, commands, fetched_at}`; `commands` is `[]` until a session advertises a catalog. This picker does not authorize command execution. |
...
Same-origin, browser-session bridges into the live sandbox. These are transports, not enumerable
REST APIs: the WebSocket bridges carry their own message protocol, and the
desktop HTTP proxy forwards noVNC paths (injecting the
provider preview credential the browser cannot set). Org API keys cannot open
these interactive transports.
