...
- `GET /api/integrations/slack/callback` and
  `GET /api/integrations/callback/slack` - one-use OAuth-state authenticated.
...
| GET | `/api/config` | Client capability contract. | Includes `auth`, `release`, dispatch-ready `engines`/`models`, discoverable `configuredEngines`/`configuredModels`, `engineReadiness`, sandbox and integration capabilities, and artifact fidelity. Public. |
| POST | `/api/config/models/refresh` | Refresh the OpenCode OpenRouter `:free` model lane. | Org-session. `429` during the refresh cooldown, `409` when the lane is managed by the qualifier, and `502` when refresh fails while the stale lane remains available. |
