...
- **Release compatibility.** Every `/api/*` response carries
  `x-skynet-release-fingerprint` (`run-events-v1:<commit>`) and
  `x-skynet-api-compat` (`run-events-v1`). A client may send
  `x-skynet-client-release`; a mutating request (`POST`, `PUT`, `PATCH`,
