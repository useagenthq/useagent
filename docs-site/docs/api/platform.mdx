...
| PUT | `/api/provider-connections/:provider/api-key` | Upsert an API-key connection (write-only). | Providers: `openai`, `anthropic`, `openrouter`, `daytona`. Daytona also requires `metadata.snapshotName`; the backend validates the key and active snapshot before saving. |
...
Tenant-owned SaaS connections behind a provider-neutral lifecycle. Slack OAuth
callbacks authenticate with a one-use state token and are public. Other callback
paths remain behind the normal organization session adapter. The `/org` connect
and disconnect variants also require an organization admin.

| Method | Path | Description | Notes |
| --- | --- | --- | --- |
| GET | `/api/integrations/slack/callback` | Current Slack OAuth callback landing. | Public, one-use `state`. `303` to the frontend on success or `/settings?integration=error` on failure. |
| GET | `/api/integrations/callback/slack` | Compatibility alias for in-flight Slack OAuth sessions. | Same behavior as the current callback path. |
| GET | `/api/integrations/callback/:provider` | Generic callback-first route for other providers. | Org-session plus callback `state`. |
