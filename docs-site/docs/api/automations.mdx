...
Encrypted named secrets for run-time tools. Values are **write-only**: they can
be set and removed but are never returned. Writes require an org admin. In
production's `gateway_only` mode, their values stay out of the sandbox;
development compatibility mode may materialize a protected dotenv or file.
