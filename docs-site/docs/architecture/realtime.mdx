...
description: See how two SSE streams, the canonicalization outbox, and the single-backend topology keep state live and replayable.
sidebar:
  order: 3
---
...
- **Watermark-stable seal.** The worker marks a run complete only when the
  ...
  snapshot.
- **Honest completeness.** A provider frame whose capture write fails even
  after a bounded retry is counted in the run's capture-loss ledger, and the
  seal then writes `complete_degraded` instead of `complete`. Both are final and
  trusted; the degraded record carries the lost-frame count on the
  `canonical-complete` frame so the client can tell the user part of the run's
  activity is missing. Ledger and seal serialize on one lock per run, so a loss
  that lands after a clean seal corrects it and announces the completion again.
  The ledger's memory copy bridges the gap until its rows land; a process
  restart before an unflushed loss lands loses that marker and the run can then
  seal clean, the one stated residual.
