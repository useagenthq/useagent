...
### Product child threads

An agent can create product-owned child work through the child-session gateway
tools. Each child is an ordinary useAgent thread with its own root run, sandbox
binding, engine, model, repositories, and durable event stream. The relationship
record names its parent and family, so the UI can show the tree without merging
child history into the parent.

Product child threads are messageable. A follow-up is a normal
`POST /api/runs` with `parent_run_id` set to a run in that child's thread. The
backend re-authorizes the parent, inherits the child thread's engine and
resources, and sends the turn through normal admission. Org API keys may use
this exact `/api/runs` route; they cannot use the browser-only
`POST /api/threads/:id/messages` convenience route.

### Provider-native children

Codex, Claude Code, OpenCode, and Pi may also report child tasks in their native
event stream. Those children render in the same execution view, but they do not
become product threads merely because a provider emitted a task id. They remain
read-only unless the native driver has a verified, persisted resume/steer
contract for that exact child session.

The distinction is intentional: a product child is addressable backend state;
a native child is provider evidence. Both can produce
`child.started/updated/completed` canonical events without sharing control
semantics.
...
| Child work | `child.started/updated/completed` |
| Usage | `usage.updated` |
...
:::note
A run with a reconciliation-capable native session can **survive a backend
restart**. The sandbox keeps executing; on restart a recovery loop re-probes the
session, streams interim events into the timeline, and adopts the real result.
Other combinations fail honestly rather than claiming recovery. See [Restart
