---
title: Concepts
description: Learn the durable run, native engine, sandbox, gateway, event, knowledge, and artifact concepts that useAgent builds on.
sidebar:
...
<Steps>
  ...
  <Step title="The inner loop is rented">
    Native drivers steer a resident engine session inside the thread's sandbox
    and translate each engine's stream into product events.
...
<CardGroup cols={2}>
  ...
  <Card title="Engines and adapters" href="/concepts/engines-and-adapters" icon="plug">
    How Codex, Claude Code, OpenCode, and Pi keep their native runtime while
    sharing one product projection.
  </Card>
