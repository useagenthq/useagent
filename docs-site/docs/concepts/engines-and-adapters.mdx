...
An engine is the rented inner loop. Skynet does not build the agent that decides
...
The translator between an engine's protocol and Skynet's thread, command, and
