...
contract. **Daytona**, **Cube** and **Box** expose the same required process, filesystem,
PTY, preview, and lifecycle boundary, then keep provider-specific behavior and
tests for native capabilities (Box has no PTY or desktop yet). Each provider is
a plugin package under `packages/sandbox-*`. Code above the contract does not
