...
Skynet is the control plane for the Loop agent platform. It turns a prompt into a
...
Skynet is built on one architectural commitment, named **VERA**: Verify-gated,
...
<CardGroup cols={3}>
  ...
  <Card title="Engines are rented, not built" icon="plug">
    ...
    Skynet owns orchestration, history, approvals, and rendering.
  </Card>
...
:::note
Skynet does not run its harness inside the sandbox, does not embed an engine's
