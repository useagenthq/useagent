...
vendor UI. That decision is what lets the product feel like one product: several
engine paths, one timeline grammar, one design system, and extension
...
The view is fed by a thread-scoped stream that hands back a durable snapshot on
reconnect, then applies live frames through the client reducer.
...
:::tip
The session UI gets its durable structure from backend run, step, native-frame,
and canonical-event records. Local view state controls presentation, while the
backend owns the work and its status.
:::
