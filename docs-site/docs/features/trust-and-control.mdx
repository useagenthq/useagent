...
description: Understand run-bound capabilities, the restricted gateway, one-shot approvals, write-only secrets, and server-enforced organization scope.
sidebar:
  order: 3
---
...
For a supported destructive gateway tool, the run pauses for the card and
resumes only with the issued capability. The full tool
