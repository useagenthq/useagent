...
UseAgent turns a prompt into a durable run. A coding engine can execute it in an
isolated Linux workstation, or the `chat` engine can answer without a sandbox.
The backend records the run and the UI follows it through one session model.
...
<CardGroup cols={2}>
  ...
  <Card title="Reliability" href="/features/reliability" icon="rotate-ccw">
    Durable commands, reconnect snapshots, per-thread dispatch, recovery where
    the engine supports it, and guarded release checks.
  </Card>
  <Card title="Engines" href="/features/engines" icon="plug">
    OpenCode, Pi, Codex, and Claude behind provider drivers, plus direct chat.
    Each path advertises the capabilities it actually supports.
  </Card>
...
:::note
**Grounding discipline.** These pages describe the current repository. They
separate code-backed behavior from deployment-dependent checks and avoid
performance or production claims that need live evidence.
:::
