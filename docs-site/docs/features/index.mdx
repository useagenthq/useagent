---
title: Features
description: Explore the agent contract, trust boundary, reliability model, native engines, workspace, learning flow, and product experience.
sidebar:
  label: Why UseAgent
  order: 1
---

For sandboxed agent work, **a prompt becomes a durable, replayable run** inside
an isolated Linux workspace, rendered from the event log and finished with
published outputs captured for review. Chat is the no-sandbox conversational
path.
...
<CardGroup cols={2}>
  ...
  <Card title="Reliability" href="/features/reliability" icon="rotate-ccw">
    Capable native sessions can be reconciled after backend restarts,
    reconnects replay durable events, and retries are idempotent.
  </Card>
  <Card title="Engines" href="/features/engines" icon="plug">
    Codex, Claude Code, OpenCode, and Pi behind native drivers. The inner loop is
  ...
  <Card title="The workspace" href="/features/workspace" icon="monitor">
    Isolated workspaces for sandboxed threads, with terminal, desktop,
    recording, repository, and editing surfaces shown only when supported.
  </Card>
...
:::note
**Grounding discipline.** These pages distinguish implemented contracts,
deployment readiness, and live verification. Read
[Verified support](/operations/verified-support) before treating an available
driver or configured provider as current end-to-end certification.
:::
