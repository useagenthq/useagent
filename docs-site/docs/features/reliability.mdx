---
title: Reliability
description: Learn how durable commands, replayable events, idempotent retries, bounded recovery, and fingerprinted live journeys protect work.
sidebar:
  order: 4
---
...
## Supported sessions survive backend restarts

The sandbox executes independently of the backend. When the exact native driver
and provider session advertise reconciliation, a backend bounce does not have to
discard the live work.
...
<figcaption>
  For a session with reconciliation support, the retained sandbox can continue
  while the backend restarts and later adopts the provider's durable result.
</figcaption>
</figure>
...
Release evidence is recorded for the exact engine, credential mode, model,
sandbox provider, runtime generation, and image fingerprint that ran. A green
deterministic suite or one provider journey is not relabeled as a full matrix.
See [Verified support](/operations/verified-support) for the current boundary
and [Verification](/operations/verification) for the required evidence shape.
