...
export const EXTRACT_MEMORIES_SYSTEM_PROMPT = `[CRITICAL LANGUAGE RULE - HIGHEST PRIORITY]
Write ALL free-text output (scene_name, memory content) in the SAME language as the user's messages. English messages MUST produce English-only output. The Chinese text below is only the instruction skeleton, not a language cue; never output Chinese unless the user's messages themselves are Chinese. JSON field names, enum values and ISO timestamps stay in English.
...
export const EXTRACT_WORK_MEMORIES_SYSTEM_PROMPT = `[CRITICAL LANGUAGE RULE - HIGHEST PRIORITY]
Write ALL free-text output (scene_name, memory content) in the SAME language as the dominant language of the source messages. English messages MUST produce English-only output. The Chinese text below is only the instruction skeleton, not a language cue; never output Chinese unless the source messages themselves are Chinese. JSON field names, enum values and ISO timestamps stay in English.
