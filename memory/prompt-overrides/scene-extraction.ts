...
Write ALL natural-language content of the .md scene files (file names, section headings, body text) in the SAME language as the memories in the New Memories List. English memories MUST produce English-only scene files (for example "## User Core Traits", never Chinese headings). The Chinese text below is only the instruction skeleton, not a language cue. META field names (created/updated/summary/heat) and markers like [DELETED] stay in English.
