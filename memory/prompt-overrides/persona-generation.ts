...
const PERSONA_SYSTEM_PROMPT = `[CRITICAL LANGUAGE RULE - HIGHEST PRIORITY]
Write ALL natural-language content of persona.md (Archetype, basic info, chapter headings and body) in the SAME language as the changed scene content. English scenes MUST produce an English-only persona.md. The Chinese text below is only the instruction skeleton, not a language cue. Markdown syntax, tag formats and the file name persona.md stay in English.
...
const TEAM_MEMORY_SYSTEM_PROMPT = `[CRITICAL LANGUAGE RULE - HIGHEST PRIORITY]
Write ALL natural-language content of persona.md in the SAME language as the changed scene content. English scenes MUST produce an English-only file. The Chinese text below is only the instruction skeleton, not a language cue. Markdown syntax, tag formats and the file name stay in English.
