...
import { extractWithLanguagePolicy, LanguagePolicyError } from "../prompts/language-policy.js";
...
async function callLlmExtraction(params: {
  ...
  const runner = llmRunner ?? new CleanContextRunner({
    config,
    modelRef: model,
    enableTools: false,
    logger,
  ...
  return extractWithLanguagePolicy({
    sourceMessages: newMessages.filter((message) => message.role === "user"),
    extract: async (correctionInstruction) => {
      const result = await runner.run({
        prompt: correctionInstruction ? `${userPrompt}\n\n${correctionInstruction}` : userPrompt,
        systemPrompt,
